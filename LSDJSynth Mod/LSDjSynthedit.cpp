...
//Pegmode new code
bool createDMW(waveTable table, bool intp, char* fname, instParam* inst)
{
	char filenamebuffer[100]; 

	static const char DMW_HEADER[] = {
		0x20, 	//number of samples per wave
		0x00,0x00,0x00,
		0xFF, //probably something to do with the spaces in between the samples
		0x00,0x00,
	};		
	for (size_t i = 0; i < 15; i++)
	{
		sprintf_s(filenamebuffer, "%s%i", fname, i + 1);
	
		FILE* f = createFileExt(filenamebuffer, "dmw", "wb");
		CHECK fwrite(DMW_HEADER, sizeof(char),sizeof(DMW_HEADER),f);

			for (size_t j = 0; j < 31; j++) {
				CHECK fputc(table[i][j] & 0x0F, f); 
				CHECK fseek(f, 3, SEEK_CUR);
			}
			CHECK fseek(f, -1, SEEK_CUR);
			CHECK fputc(0x00, f);
		fclose(f);
	}
	return true;
...
int main(int argc, char** argv)
{
	...
	for (int i = 5; i < argc; i++) {
		...
		else if (!strcmp(argv[i], "--dmw") || !strcmp(argv[i], "-d"))
			exportMode |= EXPORT_DMW;
	...
	if (exportMode & EXPORT_XPMCK)
		if (!createXPMCK(raw, Intp, fname, &waveInst))
			abort(E_EXPORT);
	if (exportMode & EXPORT_DMW)
		if (!createDMW(raw, Intp, fname, &waveInst))
			abort(E_EXPORT);
	//if (!exportMode)
		printWaves(raw, Intp);
